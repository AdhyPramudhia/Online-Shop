  ...
  constructor() {
    ...
    Drop = (id) => {
      if(window.confirm("Apakah anda yakin ingin menghapus data ini?")) {
        // $("#loading").toast("show");
        let url = "http://localhost/onlen/public/products/drop/"+id;
        axios.delete(url)
        .then(response => {
          $("#loading").toast("hide");
    ...
    Save = (event) => {
      ...
      let url = "http://localhost//public/produk/save";
    ...
    search = (event) => {
      if(event.keyCode === 13) {
        // $("#loading").toast("show");
        let url = "http://localhost/onlen/public/produk";
        let form = new FormData();
                  ...
                  { this.state.produk.map((item) => {
                    return(
                      <tr key={item.id}>
                        ...
                        <td>{item.jmlh_brg}</td>
                        <td><img src={"http://localhost/onlen/public/image/" + item.img_brg} className="card-img-top" 
